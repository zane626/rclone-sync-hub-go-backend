...
| 监听目录 | 多目录配置、独立扫描周期、深度和关键字过滤、启停、立即扫描、本地目录逐层选择、文件名上传路径管道 |
| 本地文件索引 | 持久化已扫描文件、按目录逐层浏览、上传/排队/失败/缺失等状态展示 |
| 远端路由 | 可复用 rclone 目标独立管理，监控目录通过路由绑定目标 |
| 远端文件索引 | 后台低频 `lsjson` 流式扫描、完整路径持久化、目录树浏览、缺失记录隐藏并在完整扫描后清理、手工立即扫描 |
...
| 数据治理 | 首次启动仅创建缺失业务表（不自动升级已有表）、日志/终态任务/扫描/审计数据分批保留清理 |
...
监听目录可选配置有序的 `path_pipeline`。管道以文件名（不含本地目录）为初始输入，当前支持 `regex_extract`：正则匹配当前输入，并把指定捕获组交给下一步；最后一步结果作为远端路由下的子目录。配置保存在独立表中，执行器按步骤类型注册，后续可增加日期解析、替换或外部处理步骤而无需改动扫描器。
...
例如文件 `[Zz1tai]-直播回放-[2025-11-19_21_01_27].mp4` 配合远端路径 `backup/Zz1tai`，最终任务路径为 `backup/Zz1tai/2025-11/[Zz1tai]-直播回放-[2025-11-19_21_01_27].mp4`。未匹配时保留原相对路径；非法、空值或尝试越出远端路由的提取结果会让本轮扫描明确报错且不为该文件建单。修改管道视同修改上传目标，会取消旧目标的未完成任务并重新扫描。
